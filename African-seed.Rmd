...
tax.mean.ps1_Bacteria <- taxa_sums(ps1_Bacteria)/nsamples(ps1_Bacteria) # average number of reads for each otu
sites.prune.ps1_Bacteria <- prune_taxa(tax.mean.ps1_Bacteria > prunescale, ps1_Bacteria)

#Next, we are going to format a dataframe of predictors (OTUs) and responses (States)
# Make a dataframe of training data with OTUs as column and samples as rows
predictors.ps1_Bacteria<- t(otu_table(sites.prune.ps1_Bacteria));predictors.ps1_Bacteria
dim(predictors.ps1_Bacteria)
# 1197 1655

# Make one column for our outcome/response variable 
response.ps1_Bacteria <- as.factor(sample_data(sites.prune.ps1_Bacteria)$Genotype);response.ps1_Bacteria
# Combine them into 1 data frame
rf.data.ps1_Bacteria <- data.frame(response.ps1_Bacteria, predictors.ps1_Bacteria);rf.data.ps1_Bacteria
str(rf.data.ps1_Bacteria)
# Set a random seed
set.seed(5000)
...
boot.imp.s <- foreach (Try=1:100, .combine=rbind) %dopar% {
  ...
  ps1_Bacteria.classify <- randomForest(response.ps1_Bacteria ~., data = rf.data.ps1_Bacteria, ntree = 500, proximity=T, mtry=200)
  print(ps1_Bacteria.classify)
  
  # Make a data frame with predictor names and their importance
  imp.s <- importance(ps1_Bacteria.classify)
...
boot.oob <- foreach (Try=1:100, .combine=rbind) %dopar% {
  ...
  c(boot.oob, ps1_Bacteria.classify$err.rate[500,1])
}
...
r.s <- rownames(tax_table(ps1_Bacteria)) %in% otunames.s;r.s

# Use r.s to pull out the taxonomic information for those OTUS in otunames.s
t.table <- as.data.frame(tax_table(ps1_Bacteria)[r.s, ])
write.csv(t.table, "RF.ps1_Bacteria.t.table.csv", quote=F,row.names=T)
...
sums<-summaryBy(. ~ response.ps1_Bacteria, data = rf.data.ps1_Bacteria[, c('response.ps1_Bacteria', as.character(otunames.s))], FUN = sum, keep.names = T);sums
state.organize <- sapply(as.character(otunames.s), FUN = function(x) which.max(sums[,x]));state.organize
sums$response.ps1_Bacteria
rf.data.ps1_Bacteria$response.ps1_Bacteria
# hm is a dataframe of the abundance of the predictors, standardized and log transformed
hm <- rf.data.ps1_Bacteria[,as.character(otunames.s)] %>% decostand(method='max', MARGIN=2) %>% decostand(method='log');hm
hm$sample <- gsub('_',' ',sample_data(ps1_Bacteria)[rownames(hm),'Genotype']$Genotype)
...
g1_bacteria <- ggplotGrob(plot1_bacteria);g1_bacteria
...
library(patchwork)
#Put them together
RF1_bacteria=ggarrange(g1_bacteria, g2_bacteria, align='h', nrow=1, ncol=2, widths=c(4,2));RF1_bacteria
tiff("RF1_bacteria.tiff", units="in", width=19, height=3, res=300)
RF1_bacteria.tiff=RF1_bacteria+plot_layout(nrow=1);RF1_bacteria.tiff
...
gg2=ggplot(psmelt(ps.toptax_Fungi), aes(x=reorder(Sample, Genotype), y=Abundance, fill=Order))+
  ...
  scale_fill_manual(values = mycola)+
  xlab("") + ylab("Relative abundance (%)") +
  facet_wrap(~reorder(Genotype, Sample), scales = "free_x", nrow = 3) + 
  ggtitle("Order, top 500 ASVs") +
  theme_pubclean() + theme(legend.position="bottom", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0)) +
theme(legend.text = element_text(face = c(rep("italic", 5), rep("plain", 5))))+
  guides(fill= guide_legend(keywidth = 0.6, keyheight = 0.6, nrow = 3))+
