...
#           Df SumOfSqs   R2      F Pr(>F)    
#Model     4   6.9505 0.76966   20.884  0.001 ***
#Residual 25   2.0801 0.23034                  
#Total    29   9.0306 1.00000                  
#---
#Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
###################################################################################################
# "unifrac", "wunifrac", "manhattan", "euclidean", "canberra", "bray", "kulczynski" ...(vegdist, dist)
...
#ggsave("pcaplot1_fungi.tiff", plot = pcaplot1_fungi.tiff)
# pc = c(1, 3) to show the first & third principal components.
...
toptax = names(sort(taxa_sums(ps3_Archaea), decreasing=TRUE))[1:25];toptax
ps.toptax_Archaea = transform_sample_counts(ps3_Archaea, function(x) {x/sum(x)*100});ps.toptax_Archaea
ps.toptax_Archaea <- prune_taxa(toptax, ps.toptax_Archaea);ps.toptax_Archaea
...
mt=phyloseq2meco(ps3_Bacteria);mt
...
mt=microtable$new(sample_table = mt$sample_table, otu_table = mt$otu_table, tax_table = mt$tax_table);mt
class(mt)
mt$sample_table
...
tmp_mt <- clone(mt);tmp_mt
# transpose res_spe_func_perc to be a data.frame like taxonomic abundance
tmp <- as.data.frame(t(t1$res_spe_func_perc), check.names = FALSE);tmp
...
t2 <- trans_diff$new(dataset = tmp_mt, method = "anova", group = "Genotype", taxa_level = "func");t2

Relative_bacterial_func=t2$plot_diff_abund(add_sig = T) + ggplot2::ylab("Relative abundance (%)");Relative_bacterial_func
...
network <- trans_network$new(dataset = mt, cal_cor = "base", taxa_level = "OTU", filter_thres = 0.0001, cor_method = "spearman");network
network$cal_network(p_thres = 0.05, COR_cut = 0.7)
network$cal_module()
...
Network_bacterial_func=meco_module_func$plot_spe_func_perc(order_x = paste0("M", 1:10));Network_bacterial_func

 ggsave(
   "/Users/expeditoolimi/Documents/Southampton/Tosin/African-seed-microbiome/Network_bacterial_func.tiff",     
  plot = Network_bacterial_func,
   scale = 1,
 width = 25,
  height = 20,
  units = c("cm"),
...
meco_module_func$func_group_list
# use show_prok_func to see the detailed information of prokaryotic traits
meco_module_func$show_prok_func("methanotrophy")
```
